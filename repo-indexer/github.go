...
func fetchIssuesForLanguage(repoName string, lang, sinceTimestamp string, wg *sync.WaitGroup, issueChan chan<- []Issue, errChan chan<- error) {
	defer wg.Done()

	query := fmt.Sprintf(`repo:%s label:"good first issue","help wanted","documentation","beginner","easy","first-timers-only","beginner friendly" is:issue is:open no:assignee -linked:pr language:%s`, repoName, lang)
